create or replace function fc_export_qa_rules
(
...
begin
 ...
 loop
    ...
    dbms_lob.append(l_return, '                ''' || replace(l_tab(i).qaru_created_by, '''', '''''')      || ''' as qaru_created_by,' || chr(10));
    dbms_lob.append(l_return, '                ''' || replace(l_tab(i).qaru_updated_on, '''', '''''')      || ''' as qaru_updated_on,' || chr(10));
    dbms_lob.append(l_return, '                ''' || replace(l_tab(i).qaru_updated_by, '''', '''''')      || ''' as qaru_updated_by' || chr(10));
