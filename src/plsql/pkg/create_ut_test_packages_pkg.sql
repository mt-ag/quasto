...
create or replace package body create_ut_test_packages_pkg is
  ...
  begin
    ...
    loop
      ...
      if rec_client_rules.qaru_category = 'APEX'
      then
          if rec_client_rules.qaru_layer = 'PAGE'
          then
            ...
            l_clob := l_clob || '  l_app_id := rec.application_id;' || chr(10);
            l_clob := l_clob || '  l_app_page_id := rec.page_id;' || chr(10);
            l_clob := l_clob || '  l_layer := ''' || rec_client_rules.qaru_layer || ' (Application: ''||rec.application_id||'' - Page: ''||rec.page_id||'')'';' || chr(10);

          elsif rec_client_rules.qaru_layer = 'APPLICATION'
          then
